...
public class I18nField extends CustomField<I18n> implements IRequiredField{
	...
	public I18nField(Langue langueParDefaut, List<Langue> listeLangueEnService, String libelleBtnPlus) {
		...
		btnAddLangue = new Button(libelleBtnPlus,FontAwesome.PLUS_SQUARE_O);
	...
	private HorizontalLayout getLangueLayoutInactive(I18nTraduction traductionInactive){
		...
		Button removeLangue = new Button(FontAwesome.MINUS_SQUARE_O);
	...
	private void validateFields(Boolean validate){
		listLayoutTraductions.forEach(e -> {		
			...
			if (validate){
				tf.removeStyleName(StyleConstants.FIELD_ERROR);
			}else{
				tf.addStyleName(StyleConstants.FIELD_ERROR);
			}
		});
	}
