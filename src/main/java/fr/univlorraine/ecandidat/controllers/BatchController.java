...
public class BatchController {	
	...
	/**
	 * Enregistre un batch
	 * @param batch le batch a enregistrer
	 */
			...
			if (liste!=null && liste.size()!=0){
				...
				listeBatch.forEach(batch -> {				
					...
					if (histo==null && isNeededToLaunch(batch,lastBatchRun.getDatLastCheckRun())){
						logger.trace("Le batch doit être lancé");
						runJob(batch);
					}
				});
				batchRunRepository.delete(lastBatchRun);
			}
	 ...
	 */
	public Boolean isNeededToLaunch(Batch batch, LocalDateTime lastExec){
		...
		/*Vérification si le batch doit etre lancé annuelement avec un mois donné et un jour donné*/
	...
	private void runJob(Batch batch){
		...
		try {			
			...
			}else if (batch.getCodBatch().equals(NomenclatureUtils.BATCH_SYNC_LIMESURVEY)){
	...
	private void nettoyageBatch(){
		...
		listHisto.forEach(batchHisto->{
			LocalDateTime now = LocalDateTime.now();
			LocalDateTime histPlusHour = batchHisto.getDateDebBatchHisto().plusHours(new Long(3));
