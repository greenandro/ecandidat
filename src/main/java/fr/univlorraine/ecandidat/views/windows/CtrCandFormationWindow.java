...
public class CtrCandFormationWindow extends Window {
	...
	public static final String[] FIELDS_ORDER_1 = {
		Formation_.codEtpVetApoForm.getName(),
	...
	public static final String[] FIELDS_ORDER_4 = {
		Formation_.preselectLieuForm.getName(),
		Formation_.preselectDateForm.getName(),
		Formation_.preselectHeureForm.getName()	
		};
	public static final String[] FIELDS_ORDER_5 = {
			Formation_.infoCompForm.getName()
	};
	 ...
	 * @param formation la formation à éditer
	 */
	public CtrCandFormationWindow(Formation formation, CentreCandidature ctrCand) {
		/* Style */
		setModal(true);
		...
		if (parametreController.getSIScolMode().equals(ConstanteUtils.SI_SCOL_APOGEE)){
			...
			/*Bouton importation*/
			Button btnApo = new Button(applicationContext.getMessage("formation.window.btn.apo", null, UI.getCurrent().getLocale()));		
		...
		/*Ajout du listener pour action sur la checkbox des liste comp-->activation ou desactivation des decisions fav list comp*/
		...
		if (formation.getIdForm()==null){
			((RequiredDateField)fieldGroup.getField(Formation_.datConfirmForm.getName())).setLocalValue(ctrCand.getDatConfirmCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datDebDepotForm.getName())).setLocalValue(ctrCand.getDatDebDepotCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datAnalyseForm.getName())).setLocalValue(ctrCand.getDatAnalyseCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datFinDepotForm.getName())).setLocalValue(ctrCand.getDatFinDepotCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datJuryForm.getName())).setLocalValue(ctrCand.getDatJuryCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datPubliForm.getName())).setLocalValue(ctrCand.getDatPubliCtrCand());
			((RequiredDateField)fieldGroup.getField(Formation_.datRetourForm.getName())).setLocalValue(ctrCand.getDatRetourCtrCand());
			((RequiredTextArea)fieldGroup.getField(Formation_.infoCompForm.getName())).setValue(ctrCand.getInfoCompCtrCand());
	...
	private Field<?> getField(String fieldName){
		...
		Field<?> field;
		if (fieldName.equals(Formation_.motCleForm.getName()) || fieldName.equals(Formation_.infoCompForm.getName())){
			field = fieldGroup.buildAndBind(caption, fieldName, RequiredTextArea.class);
		}else if (fieldName.equals(Formation_.codEtpVetApoForm.getName()) || fieldName.equals(Formation_.codVrsVetApoForm.getName()) || fieldName.equals(Formation_.libApoForm.getName())){
	...
	private String getErrorMessageDate(){
		...
		if (datConfirm!=null){
			 ...
			 **/
				...
				/*
				 * Date de publi a null et -->on verifie qu'elle est superieur a la date de retour 
				 * Date limite de confirmation  >= Date de publication des résultats
				 **/
