...
/**Gestion du SI Scol Apogee en 4.60
 * @author Kevin Hergalant
 *
 */
@Component(value="siScolApogeeWS460ServiceImpl")
@SuppressWarnings("unchecked")
public class SiScolApogeeWS460ServiceImpl implements SiScolGenericService, Serializable{

	/*** serialVersionUID*/
	private static final long serialVersionUID = -1491618437747654843L;

	private Logger logger = LoggerFactory.getLogger(SiScolApogeeWS460ServiceImpl.class);
	...
	public List<WSCursusInterne> getCursusInterne(String codEtu)
		...
		try {			
			...
			ContratPedagogiqueResultatVdiVetDTO[] resultatVdiVet = monProxyPedagogique
					.recupererContratPedagogiqueResultatVdiVet(codEtu,
							"toutes", "Apogee", "ET", "toutes", "tous");
				...
				for (int i = 0; i < resultatVdiVet.length; i++) {
					...
					if (etapes != null && etapes.length > 0) {
						...
						for (int j = 0; j < etapes.length; j++) {
							...
							// 29/01/10
							// on rejete les etapes annul̩e. MAJ sur proposition
							// de Rennes1
							if ((etape.getCodEtaIae() == null)
									|| (etape.getCodEtaIae() != null && !etape
											.getCodEtaIae().equals("A"))) {

								// r̩sultats de l'̩tape:
								ResultatVetDTO[] tabresetape = etape
										.getResultatVet();
								if (tabresetape != null
										&& tabresetape.length > 0) {
										...
										WSCursusInterne cursus = new WSCursusInterne(
												...
												etape.getEtape().getLibWebVet()
														...
														+ ret.getSession()
																.getLibSes(),
												etape.getCodAnu(),
												(ret.getMention() != null) ? ret
														.getMention()
														.getCodMen() : null,
												(ret.getTypResultat() != null) ? ret
														.getTypResultat()
														.getCodTre() : null);
										liste.add(cursus);
